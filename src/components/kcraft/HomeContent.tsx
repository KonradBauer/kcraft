...
const eyebrow = 'block font-montserrat text-[12px] font-semibold tracking-[0.28em] uppercase text-[#FF9228]'
...
const HERO_FEATURES = [
  { Icon: HeartIcon, label: 'Pasja w tym co tworzymy' },
  { Icon: LightbulbIcon, label: 'Funkcjonalność w każdym detalu' },
  { Icon: SettingsIcon, label: 'Solidność na lata' },
]
...
export function HomeContent({ hero, about, areas }: HomeContentProps) {
  ...
  return (
    <ModalProvider>
      ...
      <header className="relative bg-ink text-light min-h-[760px] max-[980px]:min-h-[100svh] overflow-hidden" id="top">
        ...
        <div className="absolute inset-0 z-[1] pointer-events-none max-[980px]:hidden [background:linear-gradient(to_right,rgba(33,20,7,0.97)_0%,rgba(33,20,7,0.7)_32%,rgba(33,20,7,0.15)_55%)]" />
        <div className="hidden max-[980px]:block absolute inset-0 z-[1] pointer-events-none [background:linear-gradient(to_right,rgba(33,20,7,0.99)_0%,rgba(33,20,7,0.9)_55%,rgba(33,20,7,0.65)_100%)]" />
        ...
        <div className="absolute bottom-[40px] left-0 right-0 z-[4] max-[980px]:hidden">
          <div className={wrap}>
            <div className="flex justify-between items-center max-w-[720px] mx-auto">
              {HERO_FEATURES.map(({ Icon, label }) => (
                <div key={label} className="flex items-center gap-3">
                  <Icon className="w-6 h-6 text-[#FF9228] flex-none" />
                  <span className="font-montserrat text-[13px] font-medium tracking-[0.05em] text-white">{label}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ====== ABOUT ====== */}
      <section className="bg-ink-3 relative pt-24 pb-[78px]" id="about">
        ...
        <div className={wrap}>
          <div>
            ...
            <div className="min-w-0">
              ...
              <h2 className="font-medium text-[27px] tracking-[0.02em] text-white mt-[14px] mb-[22px] uppercase">
                {LEGAL_NAME}
              </h2>
              <p className="max-w-[860px] text-[15px] leading-[1.85] text-white">
                {bioText}
              </p>
            </div>
      ...
      {/* ====== OBSZARY ====== */}
      <section className="bg-ink-3 py-[74px]" id="areas">
        <div className={wrap}>
          <div>
            <span className={`${eyebrow} mb-[14px]`}>Co oferuję?</span>
            <h2 className="font-montserrat font-semibold text-[30px] tracking-[0.04em] uppercase text-white">Obszary działalności</h2>
          </div>
      ...
      {/* ====== FOOTER ====== */}
      <footer className="bg-ink-3 text-white pt-16 pb-[26px] relative" id="contact">
        <div className={wrap}>
            ...
            <div>
              ...
              {[
                ...
                },
                {
                  ...
                  label: CONTACT.email,
                  icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-[17px] h-[17px] text-[#FF9228] flex-none"><rect x="2" y="4" width="20" height="16" rx="2" /><path d="m2 7 10 6 10-6" /></svg>,
                },
              ].map(({ href, label, icon }) => (
